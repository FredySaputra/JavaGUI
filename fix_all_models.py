...
def replace_table_model(f):
    ...
    # First, inject the class at the top if not present
...
"""
    if class_name not in content:
        content = re.sub(r'(import .*;\n\n)(public class )', r'\1' + class_def + r'\n\2', content)
        
    # Now replace the inner class instances
    ...
    # Also replace variable declarations if they were DefaultTableModel tblModel = new DefaultTableModel(...) {...}
    # We just replace DefaultTableModel tblModel = new MyTableModel(...) with DefaultTableModel tblModel = ...
    # No, Java allows DefaultTableModel tblModel = new MyTableModel(...), so that is fine!
